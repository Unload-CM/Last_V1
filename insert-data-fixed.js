...
async function main() {
  ...
  try {
    ...
    for (const department of departments) {
      console.log(`부서 추가 중: ${department.name}`);
    ...
    for (const status of statuses) {
      console.log(`상태 추가 중: ${status.name}`);
    ...
    console.log('상태 데이터 생성 완료!');

    // 우선순위 데이터 생성
    console.log('우선순위 데이터 생성 시작...');
    const priorities = [
    ...
    for (const priority of priorities) {
      console.log(`우선순위 추가 중: ${priority.name}`);
    ...
    for (const category of categories) {
      console.log(`카테고리 추가 중: ${category.name}`);
    ...
    if (managementDept) {
      console.log(`MANAGEMENT 부서 ID: ${managementDept.id}`);
      ...
      console.error('MANAGEMENT 부서를 찾을 수 없습니다!');
    }

    // 데모 직원 생성
    console.log('데모 직원 생성 시작...');
    const productionDept = await prisma.department.findUnique({ where: { name: 'PRODUCTION' } });
    
    if (productionDept) {
      console.log(`PRODUCTION 부서 ID: ${productionDept.id}`);
      console.log('김철수 계정 추가 중...');
      await prisma.employee.upsert({
      ...
      console.log('데모 직원 생성 완료!');
    } else {
      console.error('PRODUCTION 부서를 찾을 수 없습니다!');
    }
