...
import { MessageSquare, Medal, Trophy, Star } from "lucide-react";
...
export default function TopCommenters({ data = [], isLoading = false }: TopCommentersProps) {
  const { t } = useTranslation();
  
  // 차트 데이터 준비
  const chartData = data.map((commenter) => ({
    name: commenter.koreanName,
    fullName: commenter.koreanName + (commenter.thaiName ? ` (${commenter.thaiName})` : ''),
    commentCount: commenter.commentCount,
    rank: commenter.rank
  }));

  // X축 틱 값 계산: 데이터에서 최대 댓글 수를 찾아 그에 맞게 설정
  const maxComments = Math.max(...data.map(c => c.commentCount), 0);
  const tickValues = Array.from({ length: maxComments + 1 }, (_, i) => i); // 0부터 최대값까지의 정수 배열
    ...
    <Card className="border-2 border-blue-200 bg-white shadow-sm overflow-hidden">
      <CardHeader className="bg-blue-50 border-b border-blue-100 py-2 px-3">
        <CardTitle className="flex items-center gap-2 text-blue-700 text-sm">
          <Trophy className="h-4 w-4 text-yellow-500" /> {t('dashboard.topCommenters')}
        </CardTitle>
        <CardDescription className="text-xs">{t('dashboard.topCommenterDescription')}</CardDescription>
      </CardHeader>
          ...
          <>
            ...
            <div className="h-36 mb-3">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Tooltip 
                    formatter={(value) => [`${value}`, t('dashboard.commentsCount')]}
                    labelFormatter={(value) => `${value}`}
                  />
                  <Bar 
                    dataKey="commentCount" 
                    name={t('dashboard.commentsCount')}
                    fill="#8884d8"
                    shape={(props) => {
            ...
            <div className="space-y-2">
              {data.map((commenter, index) => {
                // 공동 순위에 맞는 스타일 선택
                const rankIndex = commenter.rank - 1;
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-medium text-gray-800 text-xs">
                          {commenter.koreanName}
                          {commenter.thaiName && ` (${commenter.thaiName})`}
                        </div>
                        <div className="text-xs text-gray-500">{commenter.departmentName}</div>
                      </div>
                    </div>
